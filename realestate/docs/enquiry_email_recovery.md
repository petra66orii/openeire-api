...
Run `python manage.py retry_realestate_enquiry_emails --limit 100` from the production application environment to retry pending messages. Schedule this command periodically in the deployment scheduler and monitor its output. Pending enquiries with no previous attempt run first; subsequently, those least recently attempted run first, so persistent failures cannot block newer enquiries. It is safe to run concurrently on the production PostgreSQL database: it locks an enquiry while checking and sending. Sent messages are skipped on later runs.
