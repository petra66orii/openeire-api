...
class PublicAuthThrottleTests(TestCase):
    def setUp(self):
    ...
    def test_login_endpoint_throttles(self):
        for _ in range(10):
            response = self.client.post(
                self.login_url,
                data={"username": self.user.username, "password": "StrongPass123!"},
    ...
    def test_register_endpoint_throttles(self):
        for idx in range(5):
            response = self.client.post(
                self.register_url,
                data={
                    "username": f"registerthrottle{idx}",
                    "email": f"registerthrottle{idx}@example.com",
                    "password": "StrongPass123!",
                },
            )
        ...
        blocked = self.client.post(
            self.register_url,
            data={
                "username": "registerthrottle-final",
                "email": "registerthrottle-final@example.com",
                "password": "StrongPass123!",
            },
        )
    ...
    def test_password_reset_endpoint_throttles(self):
        for _ in range(5):
            response = self.client.post(
                self.reset_url,
    ...
    def test_resend_verification_endpoint_throttles(self):
        inactive_user = User.objects.create_user(
            username="resendthrottle",
            email="resendthrottle@example.com",
            password="StrongPass123!",
            is_active=False,
        )
