...
def send_realestate_internal_notification_email(enquiry, request=None):
    def display(field_name, fallback="Not provided"):
        getter = getattr(enquiry, f"get_{field_name}_display", None)
        value = getter() if getter else getattr(enquiry, field_name, "")
    ...
    body = (
        ...
        f"Eircode: {enquiry.eircode or ('Property confirmed as having no Eircode' if enquiry.no_eircode else 'Not provided')}\n"
        ...
        "Property size / scope\n"
        f"Category: {display('property_type')}\n"
        f"Category details: {enquiry.property_type_details or 'Not applicable'}\n"
        f"Bedrooms: {display('bedroom_count')}\n"
        f"Floors: {display('floor_count')}\n"
        f"Internal floor area: {enquiry.internal_floor_area or 'Not provided'} "
        f"{display('internal_floor_area_unit', '')}\n"
        f"Secondary accommodation: {display('secondary_accommodation')} - "
        f"{enquiry.secondary_accommodation_details or 'No details'}\n"
        f"Outbuildings: {display('outbuildings')} - "
        f"{enquiry.outbuildings_details or 'No details'}\n"
        f"Grounds: {display('grounds_size')}\n"
        f"Other material features: {enquiry.property_features or 'None provided'}\n\n"
        "Preferred scheduling\n"
        f"Preference: {display('scheduling_preference')}\n"
        f"Preferred date: {_format_date(enquiry.preferred_date)}\n"
        f"Alternative date: {_format_date(enquiry.alternative_date)}\n"
        f"Time window: {display('preferred_time_window')}\n"
        "Dates remain requests until confirmed.\n\n"
        "Access / readiness\n"
        f"Occupancy: {display('occupancy_status')}\n"
        f"Access provided by: {display('access_provider')}\n"
        f"Access contact: {enquiry.access_contact_name or enquiry.name} / "
        f"{enquiry.access_contact_phone or enquiry.phone}\n"
        f"Restrictions / instructions: {enquiry.access_notes or 'None provided'}\n"
        f"Ready-on-arrival acknowledged: {'Yes' if enquiry.readiness_acknowledged else 'No'}\n\n"
        "Presenter / audio\n"
        f"Anyone on camera: {display('on_camera')}\n"
        f"Who: {enquiry.on_camera_people or 'Not applicable'}\n"
        f"Audio / microphone requirements: {enquiry.audio_requirements or 'Not applicable'}\n\n"
        "Message / notes\n"
